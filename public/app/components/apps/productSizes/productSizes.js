...
app.controller('ProductSizesCtrl', ['$scope', '$rootScope', 'DataService.api', 'MessagingService', 'authService', function ($scope, $rootScope, DS, MS, authService) {
    var vm = this;
    vm.auth = authService;
    ...
    $scope.currentProductSizesData = [];

    $scope.currentExisitingProductSizesData = [];
    ...
    $scope.productSizesGrid = {
        ...
        multiSelect: false,
        columnDefs: [
            { field: "size_name", displayName: "Size Name"},
    ...
    function getProductSizes(callback) {
        ...
        DS.getProductSizes().then(processProductSizes);
        if (callback) {
            callback();
        }
    ...
    function getExistingProductSizes(callback) {
        DS.getExistingProductSizes().then(processExistingProductSizes);
    }
    ...
    function processProductSizes(response) {
        ...
        console.log($scope.productSizes);
        //getExistingProductSizes();


    }

    function processExistingProductSizes(response) {


        var temp = $scope.productSizes;
    ...
    $scope.loadMoreProductSizes = function () {
        page = page + 1;
        var results = getProductSizes(page);

        if (results.length !== 0) {
            $scope.gridApi.infiniteScroll.saveScrollPercentage();
            $scope.productSizes = $scope.productSizes.concat(results);
    ...
    $scope.updateSingleProductSize = function () {
        // creating new Size
        if ($scope.form.user.id == null) {
            $scope.createRecord();
        } else { // updating
            DS.updateProductSize($scope.form.user)
    ...
    $scope.deleteSingleProductSize = function () {
        DS.deleteProductSize($scope.view.record)
            .then(updateSuccess, updateFailure)
            .finally(function () {
                console.log("finally");
            });
    ...
    $scope.loadDataSet = function (searchTerm) {
        ...
        if (termLength >= 4) {
            ...
            DS.findProductsBySKU(searchTerm).then(processProductSizes);
